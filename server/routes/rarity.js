...
router.get('/:projectId', authenticateToken, async (req, res) => {
  ...
  const rows = await db.all(`SELECT l.id as layer_id, l.name as layer_name, l.z_index,
            a.id as asset_id, a.filename, a.rarity_weight
            FROM layers l
            LEFT JOIN assets a ON l.id = a.layer_id
            WHERE l.project_id = ?
            ORDER BY l.z_index, a.filename`, [projectId]);
...
router.get('/:projectId/stats', authenticateToken, async (req, res) => {
  ...
  const rows = await db.all(`SELECT l.name as layer_name,
...
router.post('/:projectId/validate', authenticateToken, async (req, res) => {
  const { projectId } = req.params;
